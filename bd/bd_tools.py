...
def _kpi_history_latest(ins_id: int, kpi_id: int, report: str, price: str) -> Optional[Dict[str, Any]]:
    """
...
def _kpi_value_latest_by_names(ins_id: int, names: List[str], report_order=("r12","year")) -> Optional[float]:
...
def get_key_ratios_latest_by_ticker(ticker: str) -> Dict[str, Any]:
    t = (ticker or "").upper()
    ins_id = next((i.id for i in get_instruments() if (i.ticker or "").upper() == t), None)
    if ins_id is None:
        return {}
...
def get_quote_by_id(ins_id: int) -> Dict[str, Any]:
    ...
    row = next(
        (x for x in items if (x.get("i") == ins_id or x.get("insId") == ins_id or x.get("instrumentId") == ins_id)),
        None
    )
    if not row:
        return {}
    price = _first_non_none(row.get("price"), row.get("c"), row.get("last"), row.get("close"))
...
# ------------------------
def _kpi_history_by_id(ins_id: int, kpi_id: int, reporttype: str="year", pricetype: str="mean") -> List[Dict[str, Any]]:
...
def _latest_kpi_value(ins_id: int, kpi_id: int, reporttype: str="year") -> Optional[float]:
